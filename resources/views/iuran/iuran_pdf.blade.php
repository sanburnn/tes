	...
	<center>
		<table width="100%">
			<tr>
				<td colspan="2" class="pr-2">
					<img src="images/logo/{{ $iuran->logo ?? '' }}" width="50" height="50">
				</td>
				<td class="text-left">
					<font size="1">YAYASAN MIFTAHUL HUDA KROYA</font><br>
					<font size="2" class="text-uppercase"><strong>{{ $iuran->sekolah ?? '-' }}</strong></font><br>
					<font size="1">{{ $iuran->alamat ?? '-' }}</font><br>
					<font size="1">Telepon: {{ $iuran->telepon ?? '-' }} E-mail: {{ $iuran->email ?? '-' }} Website: {{ $iuran->website ?? '' }}</font>
				</td>
			</tr>
		</table>
		<table width="100%">
		...
		<table width="100%">
			<tr>
				...
				<td style="width: 1%;">:</td>
				<td style="width: 55%;"><font size="1">{{ $iuran->nisn ?? '-' }}</font></td>
			</tr>					
			<tr>
				<td class="text-capitalize" style="width: 15%;"><font size="1">tanggal</font></td>
				<td style="width: 1%;">:</td>
				<td style="width: 35%;"><font size="1">{{ Carbon::parse($iuran->created_at)->format('d/m/Y') ?? '-' }}</font></td>
				<td class="text-capitalize" style="width: 18%;"><font size="1">nama siswa</font></td>
				<td style="width: 1%;">:</td>
				<td class="text-capitalize" style="width: 45%;"><font size="1">{{ $iuran->siswa ?? '-' }}</font></td>
			</tr>					
			<tr>
				<td class="text-capitalize" style="width: 15%;"><font size="1">jam cetak</font></td>
				<td style="width: 1%;">:</td>
				<td style="width: 35%;"><font size="1">{{ Carbon::parse($iuran->updated_at)->format('H:i:s') ?? '-' }}</font></td>
				<td class="text-capitalize" style="width: 18%;"><font size="1">kelas</font></td>
				<td style="width: 1%;">:</td>
				<td style="width: 45%;"><font size="1">{{ $iuran->nama_kelas ?? '' }}-{{ $iuran->sub_kelas ?? '' }}</font></td>
			</tr>					
		</table>
		<table width="100%">
			<tr>
		...
		<table width="100%">
			...
			<tr>
				<td class="text-capitalize text-left">
					<font size="1">{{ $iuran->iuran ?? '-' }} TA.{{ $iuran->tahun_ajaran ?? '-' }}</font>
				</td>
				<td class="text-capitalize text-left">
					<font size="1">{{ separate($iuran->nilai) ?? '-' }}</font>
				</td>
		...
		<table width="100%">
			...
			<tr>
				<td class=" text-left" style="width: 50%">
					<font size="1"><i>{{ counted($iuran->nilai) ?? '' }} rupiah</i></font>
				</td>
